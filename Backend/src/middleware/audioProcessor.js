...
class AudioProcessor {
  ...
  static validateAudioForVoiceCloning(file) {
    ...
    if (file.size < minSizeForCloning) {
      return {
        valid: false,
        error: 'Audio file too small for voice cloning. Minimum recommended size: 500KB (about 30 seconds of speech)'
      };
    }
  ...
  static async processForVoiceCloning(inputPath, options = {}) {
    try {
      ...
      if (normalize) {
        processedPath = path.join(outputDir, `${fileName}_normalized_${timestamp}.wav`);
        await this.normalizeAudio(currentPath, processedPath);
        if (currentPath !== inputPath) fs.unlinkSync(currentPath); // Clean up intermediate file
        currentPath = processedPath;
      }

      // Step 2: Trim silence
      if (trimSilence) {
        processedPath = path.join(outputDir, `${fileName}_trimmed_${timestamp}.wav`);
      ...
      if (optimize) {
        processedPath = path.join(outputDir, `${fileName}_optimized_${timestamp}.wav`);
      ...
      console.log(`📊 Final audio: ${metadata.duration.toFixed(2)}s, ${metadata.sampleRate}Hz, ${metadata.channels}ch`);
  ...
  static async analyzeAudioQuality(filePath) {
    try {
      ...
      if (metadata.duration < 10) {
        quality = 'poor';
        recommendations.push('Audio should be at least 10 seconds long for voice cloning');
      } else if (metadata.duration < 30) {
        quality = 'fair';
        recommendations.push('For best results, use audio that is at least 30 seconds long');
      }

      // Check sample rate
      if (metadata.sampleRate < 22050) {
        quality = 'poor';
        recommendations.push('Sample rate too low. Use audio with at least 22kHz sample rate');
      } else if (metadata.sampleRate < 44100) {
        if (quality === 'good') quality = 'fair';
        recommendations.push('For best quality, use audio with 44.1kHz or higher sample rate');
      }

      // Check bitrate (if available)
      if (metadata.bitrate && metadata.bitrate < 64000) {
        quality = 'poor';
        recommendations.push('Bitrate too low. Use higher quality audio (128kbps or higher)');
      }

      // Check if stereo (mono is better for voice cloning)
      if (metadata.channels > 1) {
        recommendations.push('Mono audio works better for voice cloning than stereo');
      }
